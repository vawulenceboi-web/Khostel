-- Enhanced K-H Schema with Admin System and Agent Verification
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'terms_accepted_at' AND column_name = 'terms_accepted_at') THEN
    ALTER TABLE users ADD COLUMN terms_accepted_at TIMESTAMP;
  END IF;
END $$;

-- Create admin_actions table for tracking verification decisions
...
-- Create verification_queue table for 30-minute decision tracking
...
-- Add created_at to hostels for real-time timestamps
DO $$ 
BEGIN
...
-- Password: AdminK-H2024! (hashed with bcrypt)
INSERT INTO users (
  email, 
  password_hash, 
  first_name, 
  ...
  'admin@k-h.com',
  '$2a$10$8K1p0ZxVxVxVxVxVxVxVxeJ8K1p0ZxVxVxVxVxVxVxVxVxVxVxVxVx', -- AdminK-H2024!
  'System',
  'Administrator', 
  'admin',
  true,
...
-- View for admin dashboard (easier queries)
...
SELECT 
  ...
  vq.submitted_at,
  vq.decision_deadline,
  vq.status as queue_status,
  EXTRACT(EPOCH FROM (vq.decision_deadline - NOW()))/60 as minutes_remaining
FROM users u
JOIN verification_queue vq ON u.id = vq.agent_id
WHERE u.role = 'agent' 
  AND u.verified_status = false
  AND vq.status IN ('pending', 'under_review')
